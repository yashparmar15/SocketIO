$(document).ready(function() {
   ...
   /* var socket = io.connect('http://127.0.0.1:5000');
    ...
    $('#send_private_message').on('click', function() {
        ...
        var flag = document.getElementById("mssg").innerHTML;
        document.getElementById("mssg").innerHTML =flag+"<hr>"+"<div><b>YOU : </b>"+message_to_send+"</div>";
        private_socket.emit('private_message', {'username' : recipient, 'message' : message_to_send,'sendername':sender});
    });

    private_socket.on('new_private_message', function(mes) {
        var flag = document.getElementById("mssg").innerHTML;
        document.getElementById("mssg").innerHTML = flag+"<hr><b>"+mes['user'].toUpperCase()+" : </b>"+mes['msg'];
